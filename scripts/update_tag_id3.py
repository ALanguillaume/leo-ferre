...
def read_songs(cd_number):
  m4a_files = os.listdir(f'music/updated/cd-{cd_number}/')
  m4a_files.sort()
...
def update_songs_metadata(cd_number):
  ...
  for file, name in songs.items():
    path_file = os.path.join(f'music/updated/cd-{cd_number}/', file)
    file = mutagen.File(path_file)
    file['©nam'] = name
    file['©ART'] = 'Léo Férré'
    file['©alb'] = 'Anthologie'
    file.save()
    os.rename(path_file, f'music/updated/cd-{cd_number}/{name}_cd{cd_number}.m4a')
